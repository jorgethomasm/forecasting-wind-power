...
# TTF
failures = failures_all.with_columns(
    (pl.col("Timestamp start").shift(-1) - pl.col("Timestamp end")).shift(1).alias("TTF")    
    ).rename({"Duration":"TTR"})

failures = fo_rep.with_columns(
    pl.when(pl.col("TTF") < 0)
    .then(None)
    .otherwise(pl.col("TTF"))
    .alias("TTF")
)
...
# fo = fo_norep.with_columns(pl.col("Timestamp start").diff().alias("TBF"))
...
failures_yearly_h = failures_yearly.with_columns(    
