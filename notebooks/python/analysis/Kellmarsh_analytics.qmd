...
# TTF
fo_rep = fo_all.with_columns(
    (pl.col("Timestamp start").shift(-1) - pl.col("Timestamp end")).shift(1).alias("TTF")    
    ).rename({"Duration":"TTR"})

fo_norep = fo_rep.filter(pl.col("TTF") > pl.duration(seconds=0))
...
fo_yearly = (
    ...
    .with_columns(
        ...
        (1/(pl.col("MTTF").dt.total_seconds()/3_600)).alias("FOR")
        )
    )

fo_yearly_h = fo_yearly.with_columns(    
