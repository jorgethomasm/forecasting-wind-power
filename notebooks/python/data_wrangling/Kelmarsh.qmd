...
```{python}
#| label: Extract WT data
#| eval: true

# First, read a sample to get the original column names
# turbine_data = pl.read_csv(
#     fnames_turbine,
#     skip_lines=9, 
#     null_values="NaN", 
#     use_pyarrow=True,
...
#     null_values="NaN", 
#     use_pyarrow=True,  
#     ignore_errors=True) for fname in fnames_turbine if fname.find("2022") != -1 or fname.find("2023") !=-1]
...
df_2022_2023 = pl.concat(df_list_2022_2023)
...
# Create a mapping of original names to clean names
# column_map = {
...
# # Define schema with cleaned names
# schema = {
#     'wind_speed': pl.pl.Float64,
#     'wind_direction': pl.pl.Float64,
#     'active_power': pl.pl.Float64,
#     'reactive_power': pl.pl.Float64,
#     'generator_rpm': pl.pl.Float64,
#     # Add other columns as needed
# }

# # Read CSV with renamed columns
# df = pl.read_csv(
#     folder_path_raw / fnames_turbine[0],
#     skip_lines=9,
#     columns=list(column_map.keys()),  # Original column names
#     new_columns=list(column_map.values()),  # New clean names
#     schema=schema,
#     try_parse_dates=True,
#     null_values=["NaN", "", "NA", "null"]
# )

# list_of_dfs_turbine = [pl.read_csv(f"{folder_path_raw}\{fname}", try_parse_dates=True, skip_lines=9, null_values="NaN", use_pyarrow=True,  ignore_errors=True) for fname in fnames_turbine]

# df_test_1 = pl.read_csv(f"{folder_path_raw}/{fnames_turbine[0]}", try_parse_dates=True, skip_lines=9, null_values="NaN", use_pyarrow=True, ignore_errors=False) 

# df_test_2 = pl.read_csv(f"{folder_path_raw}/{fnames_turbine[1]}", try_parse_dates=True, skip_lines=9, null_values="NaN", use_pyarrow=True, ignore_errors=False, infer_schema=True) 

# df_turbines = pkl.concat(list_of_dfs_turbine)

# list_of_dfs_turbine = []
# for fname in fnames_turbine:
#     try:
#         df = pl.read_csv(
#             f"{folder_path_raw}\{fname}",
#             try_parse_dates=True,
#             skip_lines=9,
#             ignore_errors=True,
#             null_values=["", "NA", "null", "1.119845018386826"]  # Add common null values
#         )
#         list_of_dfs_turbine.append(df)
#     except Exception as e:
#         print(f"Error reading {fname}: {e}")
#         # Optionally inspect problematic file
#         with open(f"{folder_path_raw}\{fname}", 'r') as f:
#             print(f"First few lines of {fname}:")
