...
```{python}
#| label: fill-gaps-start-end-year

# Identify the first row of each year
first_rows = tstates.group_by("Year_wt").first().drop("Status").sort("Year")

# Create new rows with the opposite State
first_rows = first_rows.with_columns(
    (1 - pl.col("State")).alias("State"),  # Flip the State (0 -> 1, 1 -> 0)
    pl.col("Timestamp").dt.truncate("1y").alias("Timestamp")  # Floor timestamp to start of year
    )

# Identify the last row of each year
last_rows = tstates.group_by("Year_wt").last().drop("Status").sort("Year")

# TODO: Last rows introuduces 42 failures if flipped 1 -> 0

# Create new rows with the opposite State
last_rows = last_rows.with_columns(
    # (1 - pl.col("State")).alias("State"),  # Flip the State (0 -> 1, 1 -> 0)
    (pl.col("State")).alias("State"),  # Flip the State (0 -> 1, 1 -> 0)
    pl.col("Timestamp").dt.truncate("1y").dt.offset_by("1y").dt.offset_by("-1s").alias("Timestamp"),  # Floor timestamp to start of year
    pl.lit("OK").alias("Message")
    )
...
columns = tstates.columns
new_rows = new_rows.select(columns)
...
# fig.update_xaxes(
#     matches=None,
#     tickformat="%d-%b",  # Show day and month
#     dtick="M12",        # Show ticks every 12 months
#     tick0="2016-01-01", # Start from January 1st
#     showgrid=True,
...
fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1], textangle=0, xanchor='left', x=-0.06))  # Make labels horizontal
...
ttf = tstates.filter(pl.col("State") == 0).rename({"Duration": "TTF"})
ttr = tstates.filter(pl.col("State") == 1).rename({"Duration": "TTR"})
