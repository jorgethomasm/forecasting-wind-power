...
# ---- 3) Id first row per year per wt
first_rows = tstates.group_by("Year_wt").first().drop("Status").sort("Year").rename({"State": "old State"})

# Create new rows with the State last seen last year
first_rows = first_rows.join(
    last_rows.select(["Year_wt", "State"]),
    on="Year_wt",
    how="left"
).with_columns(
    pl.col("Timestamp").dt.truncate("1y").alias("Timestamp"),
    pl.lit("Like last seen last year").alias("Message")
)

first_rows = first_rows.drop("old State")

# Create new df last plus firsts
first_rows = first_rows.select(last_rows.columns)
