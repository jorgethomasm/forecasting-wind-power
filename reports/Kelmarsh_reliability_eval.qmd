...
ttf_agg_year = (
    ...
    ttf.group_by(["Year",  "wt"])
    .agg(
        pl.len().alias("State"),                
        pl.sum("TTF").alias("Total TTF [h]"),
        pl.mean("TTF").alias("MTTF [h]")                      
        )
    # .rename({"wt": "Turbine"})
    #.sort("Year_wt")     
    .sort(["Year",  "wt"])     
...
rel_param = ttf_agg_year.join(ttr_agg_year, on=["Year",  "wt"], how="left")
rel_param_ = rel_param.join(year_durations, on="Year")
