...
ttr_params <- lapply(names(ttr_lnorm_fits), function(turb) {
  ...
  dplyr::tibble(    
    Turbine = turb, meanlog = fit$estimate["meanlog"], sdlog = fit$estimate["sdlog"])}) |> bind_rows()
...
#' You can see the distribution for a fleet, i.e., the whole Wind Park!
...
gganimate::anim_save("./reports/online_assets/MCS_FOR_convergence.gif", animation = plt_turbines_U_anim)
# gganimate::animate(plt_turbines_U_anim, renderer=ffmpeg_renderer("MCS_FOR_convergence.mp4"))
...
max_min_timestamp <- sim_events_trunc |> group_by(Turbine) |> summarise(max_ts = max(timestamp)) |> arrange(max_ts) 
#print(max_min_timestamp)

time_grid_longest <- seq(min(sim_events_trunc$timestamp), ceiling_date(max(sim_events_trunc$timestamp), "10 mins"), by = "10 mins")
states_longest <- rep(NA, length.out = length(time_grid_longest))
time_grid_df_longest <- tibble(timestamp = time_grid_longest, state=states_longest)

time_grid_shortest <- seq(min(sim_events_trunc$timestamp), ceiling_date(min(max_min_timestamp$max_ts), "10 mins"), by = "10 mins")
states_shortest <- rep(NA, length.out = length(time_grid_shortest))
time_grid_df_shortest <- tibble(timestamp = time_grid_shortest, system=states_shortest)

# TODO: parallelise this function

events_list <- lapply(split(sim_events_trunc, sim_events_trunc$Turbine), function(df) {  
  df |>
  dplyr::select(Turbine, timestamp, state) |>    
  full_join(time_grid_df_longest, by="timestamp") |> 
  arrange(timestamp) |>
  tidyr::fill(state.x, .direction = "down") |>
  tidyr::fill(Turbine, .direction = "down") |>    
  rename(state = state.x)  
  }
  )

df_wide <- align_sim_events(sim_events_trunc)
df_wide$total_MW <- df_wide$total *2.05
...
sim_events_trunc_T01 <- sim_events_trunc |> dplyr::select(Turbine, timestamp, state) |> filter(Turbine == "T01")
event_times <- c(0, 344, 812, 1300)  # Times when turbine changes state
states <- c(ifelse(first(sim_events_trunc_T01$state) == 0, 1, 0), sim_events_trunc_T01$state) 
state_fun <- stepfun(sim_events_trunc_T01$timestamp, states)
aligned_states_T01 <- state_fun(time_grid_shortest)

df_test_T01 <- tibble(timestamp = time_grid_shortest, state=aligned_states_T01)

df_test_T01_from_wide <- events_wide_grid |> dplyr::select(timestamp, T01) |> mutate(state = T01)  |> dplyr::select(-T01)
```

```{r}
#| label: sys-gen-capacity-curve

events_changes <- events_wide_grid |> filter(system != dplyr::lag(system) | is.na(dplyr::lag(system)))

gen_capacity_curve <- ggplot(events_changes[5000:20000, ], aes(x = timestamp, y = system_MW)) + geom_step() +
  labs(title = "Kelmarsh: System Generation Capacity Curve", x = "Simulation Timeline", y = "Wind Farm\nCapacity\n[MW]") +
  scale_y_continuous(limits = c(0, 13), breaks = seq(0, 15, by = 2.05), minor_breaks=NULL) +
  # facet_grid(rows=Turbine) + 
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))

gen_capacity_curve

capacity_curve_plotly <- ggplotly(gen_capacity_curve)

capacity_curve_plotly
