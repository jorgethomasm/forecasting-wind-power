...
events_wide_grid <- events_wide |>
  right_join(time_grid_df_shortest, by="timestamp")


events_wide_grid <- events_wide_grid |>   
  mutate(system = rowSums(dplyr::select(starts_with("T"))), na.rm = TRUE)
    
events_wide_grid$system <-  rowSums(events_wide_grid[, 2:7], na.rm = FALSE, dims = 1)
events_wide_grid$system_MW <- events_wide_grid$system *2.05

events_test <- events_wide_grid[1:10000,]
...
ggplot(events_test, aes(x = timestamp, y = system_MW)) + geom_step()
