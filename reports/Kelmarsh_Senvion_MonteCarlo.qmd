...
```{r}
#| label: A-convergence-plots


Turbines_A <- montecarlo_events |> 
  filter(state == 1) |>
...
# mc_A_turbines <- plot(Turbines_A / (tbl_sim_param + img)) 
mc_A_turbines <- plot(Turbines_A) 
mc_A_turbines
...
time_grid_min <- seq(min(montecarlo_events$timestamp), min(max_min_ts$max_ts), by = "hour")
