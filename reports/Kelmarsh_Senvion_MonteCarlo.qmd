...
sim_start_dt <- "2023-01-01 00"
sim_res_list <- lapply(split(wt_sim_param, wt_sim_param$Turbine), function(params) {montecarlo_sim(params, n_events=15000, dt_start=sim_start_dt)})
...
plt_turbines_A <- montecarlo_events |> 
  filter(state == 1) |>
  ggplot(aes(x=timestamp, y=A_U, color=Turbine)) +  
  geom_line() +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5)) +
  labs(title="Monte Carlo Simulation - Six Senvion MM92 Wind Turbines", subtitle="Convergence of Availability per Turbine", x=paste("Years since", year(montecarlo_events$timestamp[1])), y="Availability")
...
time_grid_min <- seq(min(montecarlo_events$timestamp), min(max_min_ts$max_ts), by = "mins")
...
# 2 For each turbine, interpolate the state at each hour
# turbine_states <- montecarlo_events |>
#   dplyr::select(Turbine, timestamp, state) |>
#   group_by(Turbine) |>
#   arrange(timestamp) |>
#   tidyr::complete(timestamp = time_grid_max) |>
