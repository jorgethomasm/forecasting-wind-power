...
events_wide_grid <- events_wide |> right_join(time_grid_df_shortest, by="timestamp")

# events_wide_grid <- events_wide_grid |> mutate(system = rowSums(dplyr::select(starts_with("T"))), na.rm = TRUE)
...
gen_capacity_curve <- ggplot(events_changes[5000:20000, ], aes(x = timestamp, y = system_MW)) + geom_step() +
  labs(title = "Kelmarsh: Sytem Generation Capacity Curve", x = "Simulation Timeline", y = "Wind Farm\nCapacity\n[MW]") +
