...
time_grid <- seq(min(sim_events_trunc$timestamp), ceiling_date(max(sim_events_trunc$timestamp), "10 mins"), by = "10 mins")
states <- rep(NA, length.out = length(time_grid))

df_tgrid <- tibble(timestamp = time_grid, state=states)

# time_grid_min <- seq(min(max_min_timestamp$timestamp), min(max_min_timestamp$max_ts), by = "10 mins")
# t_grid <- seq(0, length(time_grid_min), by = 5)
# states <- rep(c(1, 0), length.out = length(test$timestamp)+1)

# test <- tibble(sim_events_trunc |> filter(Turbine == "T06")_
...
test_full <- test |>
  ...
  full_join(df_tgrid, by="timestamp") |> 
  arrange(timestamp) |>
