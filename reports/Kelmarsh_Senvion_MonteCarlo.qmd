...
plt_boxplt_years <- ggplot(relia, aes(x = Year, y = `Failure Rate`)) +
  ...
  geom_dotplot(aes(color=Turbine, fill=Turbine), dotsize = 0.3, binaxis="y", show.legend=TRUE, stackdir= "center", alpha = 0.8) +
...
plt_boxplt_wt <- ggplot(relia_2017_2021,  aes(x = Turbine, y = `Failure Rate`, colour=Turbine)) + 
  ...
  labs(y=expression(lambda ~ "[h"^{-1}*"]"), x="") +  
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5)) +
  ggtitle("Mean Failure Rate per Wind Turbine at Kelmarsh (2017-2021)")
...
frates_cte$icon <- "wind-turbine-icon-10.jpg"

# frates_cte

tbl_fr <- frates_cte |>
  rename(`Mean Failure Rate [h⁻¹]` = `mean(\`Failure Rate\`)`) |>
  gt(rowname_col = "Turbine") |>
  fmt_number(
    columns = `Mean Failure Rate [h⁻¹]`,
    decimals = 5
  ) |>
  tab_header(
    title = md("**Mean Failure Rate per Wind Turbine (2017–2021)**"),
    subtitle = "Kelmarsh Senvion M92"
  ) |>
  cols_label(
    `Mean Failure Rate [h⁻¹]` = md("**Mean Failure Rate [h⁻¹]**")
  ) |>
  tab_options(
    table.font.size = 16,
...
# plt_boxplt_years / (plt_boxplt_wt + tbl_fr)
