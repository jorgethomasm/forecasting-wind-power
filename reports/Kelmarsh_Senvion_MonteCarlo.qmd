...
# Extract parameters for T01
params_T01 <- wt_sim_param |> filter(Turbine == "T01")

lambda <- params_T01$frate      # Failure rate [h^-1]
meanlog <- params_T01$meanlog   # Lognormal meanlog for TTR
sdlog <- params_T01$sdlog       # Lognormal sdlog for TTR

set.seed(123)  # For reproducibility

n_events <- 20000  # Number of simulated failures
...
# Chronological simulation: build event timeline
event_times <- cumsum(ttf_sim + ttr_sim)
...
paste("Simulation Time:", last(sim_df$total_time/8760), "years.")
...
# Build a data frame with start and end times for each event
sim_events <- sim_df |>
  mutate(
    start_time = lag(total_time, default = 0),
    end_time = total_time
  ) |>
  select(event, ttf, ttr, start_time, end_time)

# Create a long format: alternate between state=1 (operating) and state=0 (repair)
state_df <- tibble(
  start = c(0, as.vector(rbind(cumsum(sim_df$ttf)[-n_events], cumsum(sim_df$ttf + sim_df$ttr)[-n_events]))),
  end = c(cumsum(sim_df$ttf)[-n_events], cumsum(sim_df$ttf + sim_df$ttr)[-n_events]),
  state = rep(c(1, 0), length.out = 2 * (n_events - 1))
)

# Expand to hourly time series
hourly_times <- tibble(
  hour = seq(0, floor(max(sim_df$total_time))),
)

# Assign state to each hour
hourly_times <- hourly_times |>
  mutate(
    state = sapply(hour, function(h) {
      idx <- which((h >= state_df$start) & (h < state_df$end))
      if (length(idx) > 0) state_df$state[max(idx)] else NA
    })
  )
...
# Build a data frame
sim_intercalated <- tibble(
  duration = ttf_ttr_intercalated,
  state = rep(c(1, 0), length.out = length(ttf_ttr_intercalated)) 
)

sim_intercalated <- sim_intercalated |> mutate( timestamp = ymd_h("2023-01-01 00") + seconds(c(0, cumsum(duration[-length(duration)]) * 3600)))
...
ggplot(sim_intercalated[1:500, ], aes(x = timestamp, y = state)) +
  geom_step() +
