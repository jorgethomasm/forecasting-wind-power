...
```{r}
#| label: run-mcsim-single-component
#| eval: false

sim_start_dt <- "2023-01-01 00"
df_sim <- montecarlo_sim(wt_sim_param[1,], n_events=15000, dt_start=sim_start_dt)
...
sim_start_dt <- "2023-01-01 00"
sim_res_list <- lapply(split(wt_sim_param, wt_sim_param$Turbine), function(params) {montecarlo_sim(params, n_events=20000, dt_start=sim_start_dt)})
...
```{r}
#| label: A-convergence-plot
#| eval: true

#' Post: After selecting constant failure rates for the turbine and performing a grid search to fit the best distribution to repair-times (it's 
#' never exponential...) MC should be the way-to-go nowadays... 

# Labels for plot
montecarlo_events |>
  group_by(Turbine) |>  
  filter(state == 1) |>
  summarise( A = last(A_U)) # check corresponding duration per turbine

plt_turbines_U <- montecarlo_events |> 
  filter(state == 0) |>
  ggplot(aes(x=timestamp, y=A_U*8760, color=Turbine)) +  
  geom_line() +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5)) +
  labs(title="Monte Carlo Simulation - Six Senvion MM92 Wind Turbines", subtitle="Convergence of the Forced Outage Rate index (FOR) per Turbine", x=paste("Years since", year(montecarlo_events$timestamp[1])), y="FOR\n[h/yr.]", caption="1 Year = 8760 hours.")
...
max_min_ts <- montecarlo_events |>
  group_by(Turbine) |>
  summarise(max_ts = max(timestamp))

# time_grid_max <- seq(min(montecarlo_events$timestamp), max(montecarlo_events$timestamp), by = "hour")
time_grid_min <- seq(min(montecarlo_events$timestamp), min(max_min_ts$max_ts), by = "5 mins")
t_grid <- seq(0, length(time_grid_min), by = 5)
states <- rep(c(1, 0), length.out = length(test$timestamp)+1)

test  <- montecarlo_events |> filter(Turbine == "T06")
...
montecarlo_events_mins  <- montecarlo_events |>
...
tt <- montecarlo_events |>
  group_by(Turbine) |>  
  summarise(tt = sum(duration))

probs <- montecarlo_events |>
  group_by(Turbine) |>  
  filter(state == 1) |>
