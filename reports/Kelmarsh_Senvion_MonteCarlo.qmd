...
states_longest <- rep(NA, length.out = length(time_grid))
time_grid_df_longest <- tibble(timestamp = time_grid_longest, state=states_longest)

time_grid_shortest <- seq(min(max_min_timestamp$timestamp), ceiling_date(min(max_min_timestamp$max_ts), "10 mins"), by = "10 mins")
states_shortest <- rep(NA, length.out = length(time_grid_shortest))
# t_grid <- seq(0, length(time_grid_min), by = 5)

# events <- sim_events_trunc |>
...
events <- bind_rows(events_list, .id = "Turbine")
events$state.y <- NULL


events_wide <- events |>
  select(Turbine, timestamp, state) |>
  pivot_wider(
    names_from = Turbine,
    values_from = state
  ) |>
...
# sim_events_mins  <- sim_events_trunc |> mutate(timestamp = lubridate::round_date(timestamp, unit="minutes"))
...
test_full$state.y <- NULL

test_10000 <- test_full[1:10000,]
# turbine_states <- test |>
#   dplyr::select(Turbine, timestamp, state) |>
#   group_by(Turbine) |>
#   arrange(timestamp) |>
